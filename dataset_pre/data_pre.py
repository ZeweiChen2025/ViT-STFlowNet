...
def min_max_normalization_numpy(
    ...
    """
    # Normalize input channels (0-4) using min-max scaling
    data[:, 0:5, :, :] = (data[:, 0:5, :, :] - min_val) / (max_val - min_val)

    # Dimensionless normalization for output channels (5-7)
    ...
    data[:, 5:6, :, :] = (data[:, 5:6, :, :] - reference_p) / (
            0.5 * reference_rou * reference_u ** 2
    )

    # Velocity component normalization
    data[:, 6:7, :, :] /= reference_u
    data[:, 7:8, :, :] /= reference_u

    return data


def revert_min_max_normalization(
    ...
    """
    Reverse the normalization process to recover original values.

    Args:
        normalized_data: Normalized input data
        ma: Mach number
        max_val: Maximum values used in normalization
        min_val: Minimum values used in normalization

    Returns:
        np.ndarray: Data with original scale
    """
    # Reverse min-max normalization for input channels
    normalized_data[:, 0:5, :, :] = (
            normalized_data[:, 0:5, :, :] * (max_val - min_val) + min_val
    )

    # Reference values for dimensionless outputs
    reference_u = 0.1 * ma * 328.3929
    reference_rou = 0.904773
    reference_p = 69694.6

    # Reverse normalization for output channels
    normalized_data[:, 5:6, :, :] = (
            normalized_data[:, 5:6, :, :] * 0.5 * reference_rou * reference_u ** 2
            + reference_p
    )
    normalized_data[:, 6:7, :, :] *= reference_u
    normalized_data[:, 7:8, :, :] *= reference_u
...
class DataPreprocessor:
    ...
    def process_dat_files(self) -> np.ndarray:
        ...
        """
        data_list = []
        npy_file = os.path.join(self.data_file, f"dat/{self.ma}")
    ...
    def save_8channel_data(self, data: np.ndarray) -> None:
        """
        ...
        data_8c = np.concatenate(
            (data_6c[:, 0:3, :, :], fM_x, fM_y, data_6c[:, 3:, :, :]),
            axis=1
        )
        ...
        output_path = os.path.join(
            self.data_file,
            "8C_dat",
            f"Ma{self.ma:02d}_t400_8C.npy"
        )
        np.save(output_path, data_8c)
        print(f"Ma:{0.01 * self.ma} 8C data saved successfully.")


def create_10channel_normalized_data(data_file: str) -> None:
    """
    ...
    for ma in range(30, 60):
        data_path = os.path.join(data_file, "8C_dat", f"Ma{ma:02d}_t400_8C.npy")
        data_8c = np.load(data_path)

        if ma == 30:
            min_val = np.min(data_8c[:, 0:5, :, :], axis=(0, 2, 3), keepdims=True)
            max_val = np.max(data_8c[:, 0:5, :, :], axis=(0, 2, 3), keepdims=True)
        else:
            current_min = np.min(data_8c[:, 0:5, :, :], axis=(0, 2, 3), keepdims=True)
            current_max = np.max(data_8c[:, 0:5, :, :], axis=(0, 2, 3), keepdims=True)
    ...
    np.save(os.path.join(data_file, "10C_dat", "Max_val.npy"), max_val)
    np.save(os.path.join(data_file, "10C_dat", "Min_val.npy"), min_val)
    ...
    for ma in range(30, 60):
        ...
        # Normalize data
        normalized = min_max_normalization_numpy(data_8c, ma, max_val, min_val)

        # Add repeated x,y coordinates from first column
        xy_coords = normalized[:, 0:2, :, 0:1]
        xy_coords = np.repeat(xy_coords, 64, axis=3)

        # Create 10-channel data
        data_10c = np.concatenate(
            (normalized[:, 0:2, :, :], xy_coords, normalized[:, 2:, :, :]),
            axis=1
        )
        ...
        output_path = os.path.join(
            data_file,
            "10C_dat",
            f"Ma{ma:02d}_t400_10C.npy"
        )
        np.save(output_path, data_10c)
...
if __name__ == "__main__":
    ...
    for ma in range(30, 60):
        ...
        processor.save_8channel_data(raw_data)

    # Create normalized 10-channel data
    create_10channel_normalized_data(DATA_ROOT)
